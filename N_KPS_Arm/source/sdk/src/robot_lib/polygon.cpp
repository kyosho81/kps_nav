...
/*void polygon::init(F32 fangle,F32 fdis)
{
...
*/
void polygon::init_shape( const std::string &str_shape ){
...
void polygon::init_shape( const std::vector<Sxy> &shape )
{
...
void polygon::get_range(SLaser& min_range,F32 dx,F32 dy){
...
void polygon::clean()
{
	memset(angle_list_,0,sizeof(F32)*POLYGON_COUNT);
	memset(range_list_,0,sizeof(F32)*POLYGON_COUNT);
}
bool polygon::getCrossLine(Line &ln,VecPosition ray,std::vector<Sxy>::iterator &seg_end){
...
bool polygon::find_ray_in_seg(std::vector<Sxy>::iterator &it_begin,std::vector<Sxy>::iterator &it_end,const VecPosition& ray){
...
bool polygon::getCrossLine2( Line &ln,VecPosition ray )
{
...
void polygon::create_angle_range()
{
#ifdef POLYGON_DEBUG
	std::ofstream off("polygon_angle_range.txt");
#endif
	
	std::vector<Sxy>::iterator itend = shape_list_.begin();
	itend++;
	for (int i = 0 ; i < POLYGON_COUNT ; ++i)
	{
		F64 start_angle = POLYGON_ANGLE_START + POLYGON_ANGLE_DIFF*i;
		VecPosition ray(1.0,start_angle,POLAR);
		Line ray_line = Line::makeLineFromPositionAndAngle(VecPosition(0,0),ray.getDirection());
		Line seg_line;
		if (getCrossLine(seg_line,ray,itend))
		{
			VecPosition intersection;
			ray_line.getIntersection(seg_line,intersection);
			angle_list_[i] = intersection.getDirection();
			range_list_[i] = intersection.getMagnitude();

#ifdef POLYGON_DEBUG
			off<<"polygon_angle_range: "<<angle_list_[i]<<" "<<range_list_[i]<<" "<<intersection.getX()<<" "<<intersection.getY()<<std::endl;
#endif
