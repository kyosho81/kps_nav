...
void polygon_ex2::init_shape( const std::string &str_shape ){
...
void polygon_ex2::init_shape( const std::vector<Sxy> &shape )
{
	std::cout<<"begin init polygon ex2 para pos size:"<<shape.size()<<std::endl;
	if (shape.size() < 4)
	{
		Sxy xy;
		xy.x_ = 0.5;
...
void polygon_ex2::get_range(SLaser_Ex2 &min_range,F32 dx,F32 dy){
	std::cout<<"get range ex 2begin!"<<std::endl;
	//clear all data
	memset(min_range.data_,0,sizeof(F32)*LASER_COUNT_EX2);
	memset(min_range.intensities_,0,sizeof(U32)*LASER_COUNT_EX2);
	memset(min_range.seg_,0,sizeof(U8)*LASER_COUNT_EX2);
	...
	assert(min_range.used_ray_ <= LASER_COUNT_EX2);
	...
	F32 last_range = 0;
 	...
 	std::cout<<"get range ex2 end!"<<std::endl;
}

void polygon_ex2::clean()
{
	memset(angle_list_,0,sizeof(F32)*POLYGON_EX2_COUNT);
	memset(range_list_,0,sizeof(F32)*POLYGON_EX2_COUNT);
}
bool polygon_ex2::getCrossLine(Line &ln,VecPosition ray,std::vector<Sxy>::iterator &seg_end){
...
bool polygon_ex2::find_ray_in_seg(std::vector<Sxy>::iterator &it_begin,std::vector<Sxy>::iterator &it_end,const VecPosition& ray){
...
bool polygon_ex2::getCrossLine2( Line &ln,VecPosition ray )
{
