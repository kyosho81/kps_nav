...
void QuadTree<T>::insert (vertex v, T data, QTNode<T>* node, unsigned depth)
{
	...
	if (node->leaf){
		...
		if (node->bucket.size() < maxBucketSize){
			node->bucket.push_back (std::make_pair(v, data));
		}
		...
		// clear the current node's bucket and make it a stem
			...
			for (int i=0; i < node->bucket.size(); ++i){
				insert (node->bucket[i].first, node->bucket[i].second, childNode(node->bucket[i].first, node), depth+1);
			}
			node->bucket.clear();
		}
	}
...
template <typename T>
void QuadTree<T>::reduce (std::stack <QTNode<T>*>& nodes)
{
...
template <typename T>
void QuadTree<T>::print (QTNode<T>* node, std::stringstream& ss)
{
	for (int i=0; i < 4; ++i){
		if (node->child[i]){
...
template <typename T>
std::vector <std::pair <vertex, T> > QuadTree<T>::getObjectsInRegion (vertex minXY, vertex maxXY)
{
	std::vector <std::pair <vertex, T> > results;
	std::queue <QTNode<T>*> nodes;
	nodes.push (root);
...
template <typename T>
void QuadTree<T>::addAllPointsToResults (QTNode<T>* node, std::vector <std::pair <vertex, T> >& results)
{
	if (node->leaf){
...
{
	int enclosedPts = 0;
	enclosedPts += pointInRegion (vertex(center.x-range.x, center.y-range.y), minXY, maxXY);
	enclosedPts += pointInRegion (vertex(center.x-range.x, center.y+range.y), minXY, maxXY);
	enclosedPts += pointInRegion (vertex(center.x+range.x, center.y-range.y), minXY, maxXY);
	enclosedPts += pointInRegion (vertex(center.x+range.x, center.y+range.y), minXY, maxXY);
	...
	if (enclosedPts == 4){
		...
		enclosedPts += pointInRegion (vertex(minXY.x, maxXY.y), nodeMin, nodeMax);
		enclosedPts += pointInRegion (maxXY, nodeMin, nodeMax);
		enclosedPts += pointInRegion (vertex(maxXY.x, minXY.y), nodeMin, nodeMax);
		if (enclosedPts > 0){
			return NODE_PARTIALLY_IN_REGION;
		}
	}
