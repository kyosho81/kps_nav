...
public:
	...
	void stop();
	void set_ms_loop( int ms_loop );

	bool set_imu(const SIMURION &imu);
...
private:
	...
	SPos cal_odom_;						 //robot odom
	CSBuffer<SPos,1> tb_odom_;
...
private:
	U64 last_imu_stamp_;
	F32 last_yaw_;
	THSafe<SIMURION> th_imu_;

	bool check_imu_time_stamp(SIMURION imu);
	bool call_imu( F32 &dth, F32 vx, F32 vy, F32 vw,F32 cx, F32 cy, F32 cw );
	int i_countinue_;
};
