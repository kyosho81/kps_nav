...
class LogFileInfoA{
public:
	LogFileInfoA(std::ofstream *s , std::string name , long last_time){
		stream_ = s;
...
public:
	...
	void addFileLogEnable(std::string name,bool const_name = true, std::string dir = "null" , long last_time = -1) {
		if (m_file_log_enable.find(name) == m_file_log_enable.end()) {
			...
			boost::shared_ptr<LogFileInfoA> info = boost::shared_ptr<LogFileInfoA>(new LogFileInfoA(ss ,dir_name + name , last_time));
	...
	void removeFileLogEnable(std::string name) {
		std::map<std::string, boost::shared_ptr<LogFileInfoA> >::iterator it;
...
protected:
	...
	std::map<std::string, boost::shared_ptr<LogFileInfoA> > m_file_log_enable;
