...
class polar_para{
public:
	polar_para(int PM_L_POINTS):
    act(PM_L_POINTS),
	ref(PM_L_POINTS)
	{
		C = 70*70;//weighting factor; see dudek00
		small_corr_cnt=0;
		dx = 0;
		dy = 0;
		dth = 0;
...
public:
	PMScan    act;  
	PMScan    ref;//copies of actual and reference scans
	PM_TYPE   rx,ry,rth,ax,ay,ath;//robot pos at ref and actual scans
	PM_TYPE   t13,t23;

	PM_TYPE   C;//weighting factor; see dudek00
	int       iter;
	int       small_corr_cnt;
	PM_TYPE   dx;
	PM_TYPE   dy;
	PM_TYPE   dth;//match error, actual scan corrections
	PM_TYPE   avg_err;
};
class PolarMatcher
{
  private:
    ...
    void pm_scan_project(PMScan *act,  PM_TYPE   *new_r,  int *new_bad);
...
//                               PM_TYPE x3, PM_TYPE y3,PM_TYPE *x, PM_TYPE *y);
  ...
  public:
    ...
    void pm_init(const SLaser &scan);
	...
	bool pm_psm_step(PMScan *lsr,PMScan *lsa, bool b_first);
